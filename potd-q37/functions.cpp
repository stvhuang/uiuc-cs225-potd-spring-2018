...
bool validUsername(string inputUser) {
    ...
    vecCopy.pop_back();
    if (vecCopy.size() == 0) {
        cout << "Your username was too short. Try again, ol' buddy ol' pal..."
             << endl;
        return false;
    }
...
int arrayIndex(int idx) {
    ...
    if (idx >= len) {
        cout << "This index is out of bounds. Please provide a valid array "
                "index..."
             << endl;
        exit(1);
    } else {
...
int magnaCalca(int first, int second) {
    if (abs(first) > 100 || abs(second) > 100) {
        cout << "Please insert integers between -100 and 100 (inclusive)."
             << endl;
        exit(1);
    }
